...
const descriptions = [
    ...
    "제 1장. 총 칙\n" +
    "제1조(목적)\n" +
    "본 약관은 BIBID페이 주식회사(이하 \"회사\"라 합니다)가 제공하는 전자지급결제대행서비스 및 결제대금예치서비스, 선불전자지급수단 발행 및 관리 서비스(이하 통칭하여 “전자금융거래서비스”라 합니다)를 이용자가 이용함에 있어 회사와 이용자 사이의 전자금융거래에 관한 기본적인 사항을 정함을 목적으로 합니다.\n" +
    "\n" +
    "제2조 (용어의 정의)\n" +
    ...
    "③ 이용자는 회사의 거래지시에 따라 이용자의 계좌 원장에 출금기록이 기입되기 전까지 회사에 대하여 전항의 규정에 따른 동의의 철회를 요청할 수 있습니다. 이용자는 BIBID페이 서비스 화면을 통하여 회사에 등록된 계좌를 말소하는 방법으로 출금이체 동의를 철회할 수 있습니다. 다만, 이용자는 동의 철회의 의사표시 이전에 발생한 출금건에 대하여 이의를 제기할 수 없습니다\n" +
    ...
    "* 주소 : 서울특별시 송파구 송파대로 570, 16층 BIBID페이 주식회사\n" +
    ...
    "1. ‘선불전자지급수단’이라 함은 인터넷사이트 등에서 이용되는 BIBID캐시 등 회사가 발행 당시 미리 이용자에게 공지한 전자금융거래법상 선불전자지급수단을 말합니다. 회사가 발행한 각종 선불전자지급수단의 상세 내용은 인터넷사이트 등의 해당 서비스 화면을 통하여 확인할 수 있습니다.\n" +
...
보유 및 이용 기간: 회원 탈퇴 시 90일간 보관 후 파기
아이디(이메일), 이름, 휴대폰번호, 서비스 이용기록, 연령 및 성별

목적: 개인화 서비스 제공
보유 및 이용 기간: 해당 내용 미기재
부정행위 탐지된 아이디(이메일), 이름, 휴대폰번호, 부정행위 기록

목적: 부정행위 방지
보유 및 이용 기간: 회원 탈퇴 시 180일간 보관 후 파기`,

    `개인정보 제공 받는 자: BIBID페이 주식회사

 - 개인정보 제공 항목: BIBID 아이디
 
 - 개인정보 제공 목적: BIBID캐시 서비스 제공
 
 - 이용 기간: 전자상거래법에 의해 5년간 보관 후 파기
제공에 동의하지 않을 수 있으나, 동의하지 않으면 회원 가입에 제한이 됩니다.`,

    `수집 항목:

서비스 이용기록, 연령 및 성별
 - 목적: 인구통계학적 특성과 이용자의 관심 및 성향을 추정하여 맞춤형 광고에 활용
 - 보유 및 이용 기간: 동의 철회 시 맞춤형 광고 차단, 회원 탈퇴 시 파기

수집 항목: 아이디(이메일), 휴대폰번호
 - 목적: BIBID 아이디를 이용하는 서비스의 마케팅 및 프로모션
 - 보유 및 이용 기간: 회원 탈퇴 시 파기
 
연령 및 성별 정보는 회원 가입 이후 본인 확인을 진행한 이용자에 한해 활용됩니다. 동의를 거부할 수 있으며, 동의를 거부하셔도 서비스를 이용하실 수 있습니다.
    `,
    `BIBID는 회원님이 수집 및 이용에 동의한 개인정보를 이용하여 SMS(MMS), SNS 메시지, 이메일, 푸시 알림 등 다양한 전자적 전송 매체를 통해 오전 8시부터 오후 9시까지 개인 맞춤형 광고 및 정보를 전송할 수 있습니다.

본 동의는 거부하실 수 있으나, 거부 시 이벤트 및 프로모션 안내, 유용한 광고를 받아보실 수 없습니다.
광고성정보수신의 변경은

1) BIBID 고객센터(1577-7011, help@coupang.com)로 요청하거나
2) SMS, 이메일: 웹 브라우저에서 BIBID에 로그인 > MY BIBID > MY정보(개인정보확인/수정) > 수신설정
3) SNS: 각 SNS의 설정 내 수신 차단 메뉴
4) 푸시알림: BIBID 앱 로그인> MY BIBID> 이름 선택(내정보관리) > 푸시 설정에서 언제든지 변경할 수 있습니다.`
];

const ModalContent = ({ title, index }) => {
    return (
        <Box sx={{ p: 4}}>
